...
export function generateTasteProfile(
  ...
  if (topGenres.includes('Action') && topGenres.includes('Fantasy')) {
    archetype = mediaType === 'MANGA' ? 'ShÅnen Page-Turner' : 'ShÅnen Strategist';
...
function parseUserPrompt(prompt: string): {
  ...
  let seedSearchQuery = '';
  const matchLike = lower.match(/(?:like|similar to|after|vibes like)\s+([a-zA-Z0-9\s:â€™'-]{3,30})/);
...
export async function askKuroSensei(
  ...
  const results: AiRecommendationResult[] = selectedSlice.map((media, idx) => {
    ...
    if (mode === 'custom' && userPrompt.trim()) {
      ...
      vibe = `${primaryGenre} â€¢ ${media.averageScore ? media.averageScore + '% Acclaimed' : 'Community Gem'}`;
    } else if (mode === 'taste') {
      ...
      vibe = `Taste Matrix Match â€¢ ${primaryGenre}`;
      ...
      vibe = `Overlooked Masterpiece â€¢ ${scoreVal}% Score`;
    } else if (mode === 'hype') {
      ...
      vibe = `High Octane â€¢ Adrenaline Peak`;
      ...
      vibe = `Heartfelt Narrative â€¢ Tearjerker`;
      ...
      vibe = `Sweet Romance â€¢ High Chemistry`;
    } else if (mode === 'dark') {
